...
async def discover_nearby_attractions(dest: str, lat: float = None, lon: float = None, place_types: list[str] = None) -> list[dict]:
    ...
    if len(deduped) < 12:
        ...
        if not matched_fallback and state_dest:
    ...
    def get_priority_score(place):
        ...
        tourist_attraction_keywords = [
            "waterfall", "falls", "canyon", "cave", "root bridge", "living root", "double decker", 
            "boating point", "viewpoint", "view point", "valley", "trek", "trekking", "camping"
        ]
    ...
    deduped = [p for p in deduped if get_priority_score(p) != 6]
